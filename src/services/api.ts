...
class DBManagementAPI {
  ...
  async getInstanceUsage(): Promise<InstanceUsage[]> {
    const { data } = await this.client.get<InstanceUsage[]>('/usage/instance')
    return Array.isArray(data) ? data : data.data || []
  }

  async getUserUsage(): Promise<any[]> {
    const { data } = await this.client.get<any>('/usage/user')
    return Array.isArray(data) ? data : data.data || []
  }
...
export const dbAPI = new DBManagementAPI(
  process.env.REACT_APP_API_BASE_URL || import.meta.env.VITE_API_BASE_URL || '/api'
)
